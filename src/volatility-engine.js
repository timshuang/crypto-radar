...
class VolatilityEngine {
  ...
  _isTrackedSpotSymbol(symbol) {
    return ['BTCUSDT', 'ETHUSDT', 'TIAUSDT'].includes(String(symbol || '').toUpperCase());
  }

  _shouldLogSpotCheck(symbol) {
  ...
  _logTrackedAlpha(stage, payload = {}) {
    const entries = Object.entries(payload)
      ...
      .join(', ');
    console.log(`[Volatility][TrackedAlpha] stage=${stage}${entries ? `, ${entries}` : ''}`);
  }
  ...
  _logTrackedSpot(stage, payload = {}) {
    const entries = Object.entries(payload)
      .map(([key, value]) => `${key}=${value}`)
      .join(', ');
    console.log(`[Volatility][TrackedSpot] stage=${stage}${entries ? `, ${entries}` : ''}`);
  }
  ...
  /**
   * 获取 Alpha 全量币种列表(包含 symbol、alphaId、displayName)
   * 优先从 wsConnector.symbolCache 获取(WebSocket 全量推送时动态建立)
   * @returns {Array<{symbol: string, alphaId: string, source: string, displayName: string}>}
   */
  async _getAlphaSymbols() {
    ...
    if (this.wsConnector && this.wsConnector.symbolCache && this.wsConnector.symbolCache.size > 0) {
      ...
      for (const [alphaId, symbol] of this.wsConnector.symbolCache.entries()) {
        const displayName = this.storage.getSymbolForCa(alphaId) || symbol;
  ...
  async _runCheck() {
    ...
    try {
      ...
      for (const symbolConfig of volatilitySymbols) {
        ...
        const alphaId = symbolConfig.alphaId;
        let displayName = (source === 'alpha' && alphaId)
          ? (this.storage.getSymbolForCa(alphaId) || symbolConfig.displayName || symbol)
        ...
        if (source === 'spot' && this._shouldLogSpotCheck(symbol)) {
          this._logTrackedSpot('runCheck.latestPrice', {
            symbol: displayName,
            rawSymbol: symbol,
            key: priceKey,
            hasLatestPrice: !!latestPrice,
            price: latestPrice?.price ?? 'N/A',
        ...
        if (source === 'spot' && this._shouldLogSpotCheck(symbol)) {
          this._logTrackedSpot('runCheck.check', {
            symbol: displayName,
            rawSymbol: symbol,
            key: priceKey,
            hasResult: !!volatilityResult,
            isTriggered: !!volatilityResult?.isTriggered,
            volatility: volatilityResult?.volatility?.toFixed?.(2) || 'N/A',
            threshold: volatilityResult?.threshold || thresholdPercent,
            windowMinutes
          });
        }
  ...
  async handlePriceUpdate(update) {
    if (!this._flowStats || this._flowStats.second !== Math.floor(Date.now() / 1000)) {
      this._flowStats = {
    ...
    this._flowStats.entered++;
    if (update.source === 'alpha') {
      this._flowStats.alphaEntered++;
    } else {
      this._flowStats.spotEntered++;
    }

    if (this._isTrackedAlphaSymbol(update.symbol)) {
      this._logTrackedAlpha('handlePriceUpdate.enter', {
        key: update.key,
    ...
    if (volatilityResult) {
      this._flowStats.withResult++;
    }

    if (this._isTrackedAlphaSymbol(update.symbol)) {
      this._logTrackedAlpha('handlePriceUpdate.check', {
        key: priceKey,
        symbol: update.symbol,
        hasResult: !!volatilityResult,
        isTriggered: !!volatilityResult?.isTriggered,
    ...
    if (update.source === 'spot' && this._shouldLogSpotCheck(update.symbol)) {
      this._logTrackedSpot('handlePriceUpdate.check', {
        key: priceKey,
        symbol: update.symbol,
        hasResult: !!volatilityResult,
        isTriggered: !!volatilityResult?.isTriggered,
    ...
    if (volatilityResult && volatilityResult.isTriggered) {
      ...
      if (update.source === 'spot' && this._shouldLogSpotCheck(update.symbol)) {
        this._logTrackedSpot('handlePriceUpdate.trigger', {
          key: priceKey,
          symbol: update.symbol,
          normalizedSymbol,
          volatility: volatilityResult.volatility?.toFixed(2) || 'N/A'
      ...
      if (this._isTrackedAlphaSymbol(update.symbol)) {
        this._logTrackedAlpha('handlePriceUpdate.trigger', {
          key: priceKey,
          symbol: update.symbol,
          normalizedSymbol,
          volatility: volatilityResult.volatility?.toFixed(2) || 'N/A'
