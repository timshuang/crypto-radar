...
## 2026-04-22 01:57 - 波动侦测新增 3 分钟平均交易额过滤

### 本次版本
- `v1.1.8`

### 背景
- 为波动侦测增加低流动性过滤，避免一些达到涨跌幅阈值但实际成交额过低的标的继续推送，减少“无效目标”噪音。
- 这次改动要求尽量少影响原有波动告警主链路，只在新增的成交额校验链路上做补充。

### 最终规则
- 使用固定 `3m` K 线的 `quoteVolume / 3` 作为“近 3 分钟平均交易额”。
- 当波动阈值 `< 5%` 时，跳过成交额校验，按原有逻辑直接走告警。
- 当波动阈值 `>= 5%` 时，进行成交额校验。
- 若近 3 分钟平均交易额 `< minAvgQuoteVolume3m`，则不推送。
- 若满足条件，则推送消息，并在 TG/Bark 文案中追加平均交易额。
- 继续沿用现有静默期逻辑，不额外增加缓存。
...
- 设置页新增字段：`近3分钟平均交易额`
  - 预设值：`15 / 50 / 100 / 自定义`
  - 默认值：`50`
- 与现有波动窗口、波动阈值保持一致的读取 / 保存方式。
- Tooltip 文案与样式做了统一：
  - `涨跌幅设定小于 5% 时，跳过 3 分钟平均成交额校验`
  - `最近 3 分钟平均成交额未达到设定值时，不推送消息`
- Tooltip 调整为单行、即时显示、问号圆点样式。

#### 后端
- `config.volatilityModule.minAvgQuoteVolume3m` 增加默认值 `50`。
- 波动触发流程改为：
  1. 先检查静默期
  2. 再按阈值判断是否需要做 3m 成交额校验
  3. 校验通过才进入实际推送
- spot 使用 Binance spot klines：
  - `https://api.binance.com/api/v3/klines?symbol=XXXUSDT&interval=3m&limit=1`
- alpha 使用 Binance alpha klines：
  - `https://www.binance.com/bapi/defi/v1/public/alpha-trade/klines?symbol=ALPHA_xxxUSDT&interval=3m&limit=1`

### 最小侵入原则
- 保持原有 `result.symbol` 语义不变，避免影响：
  - 静默 key
  - 展示名
  - 原有通知链路
- 新增 `volumeCheckSymbol` 仅供“成交额查询链路”使用：
  - spot 用完整交易对（如 `HIGHUSDT`）
  - alpha 用内部 key（如 `ALPHA_123`）
- 这样把新增功能限制在独立查询链路内，避免污染既有 symbol / 映射逻辑。
...
### 日志补充
- 新增“校验通过”日志，方便核对“该发是否已发”：
  - `XXX 3分钟平均交易额 73.34 >= 50，允许推送`
- 原有“不达标不推送”日志保留：
  - `XXX 3分钟平均交易额 31.71 < 50，不推送`

### 验证结果
- 已确认重启后进入稳定状态：
  - 不再出现大面积 `HTTP 400` / `无有效 3m K线数据`
  - 达标标的正常推送
  - 不达标标的明确记录为交易额不足
- 示例：
  - `[alpha] TIMI 5min 下跌 9.42%（0.001115 → 0.00101） 近3分钟平均交易额：73.34 usdt`
  - `[alpha] XPIN 5min 下跌 5.47%（0.00121 → 0.001143） 近3分钟平均交易额：95.69 usdt`
  - `[alpha] NAORIS 5min 上涨 5.56%（0.06042 → 0.06378） 近3分钟平均交易额：117.9 usdt`
  - `[现货] HIGH 5min 上涨 5.26%（0.285 → 0.3） 近3分钟平均交易额：13144 usdt`

### 备注
- `tmp_probe_volume.js` 仅为临时排查文件，未纳入正式 release commit。

## 2026-04-17 14:33 - Alpha 波动告警半截名称问题记录

### 现象
- Alpha 波动告警中偶发出现 `ALPHA_823U`、`ALPHA_332U` 这类半截显示名。

### 当前判断
- 这不是消息模板层的问题，告警发出前 `symbol` 已经是半截值。
- 问题更像出在 Alpha 全量流的 symbol 解析 / 回退链路，某些情况下没有稳定落到真实币名，导致原始交易对名或异常中间值进入后续波动告警链路。

### 待继续验证
- 后续需抓取 Alpha 全量流原始 `token.s`，确认是否存在类似 `ALPHA_332UUSDT` 这样的上游原始值。
- 需要把“上游原始数据异常”和“本地解析回退异常”明确区分，避免误判根因。

### 补充记录：日志增长与后续策略
- 当前 `logs/app.log` 增长较快，主要高频来源集中在 WebSocket、Storage、Volatility 三条链路。
- 已确认一批更偏调试/链路追踪性质的日志长期常开，包括：`[WS][TrackedAlpha]`、`[Storage][TrackedAlpha]`、`[Volatility][TrackedAlpha]`、`[Volatility][TrackedSpot]` 等。
- 当前决策：暂时不立即降噪，先继续开发；待后续功能开发接近收尾后，再统一回头处理日志治理。
- 后续日志治理方向：
  1. 关闭或仅在 debug 模式下保留高频链路追踪日志
  2. 对 `symbolCache 更新`、`实时触发`、`静默期跳过` 这类日志做降频
  3. 引入日志轮转策略，避免单个日志文件无限增长
- 日志轮转建议先记录为候选方案：按文件大小轮转（例如 20MB 或 50MB 一份），保留最近 5 到 10 份。
- 已补充日志增长量化结果（以 `logs/app.log` 为准）：
  - 整体平均约 `0.93 MB/小时`
  - 历史高峰约 `26.33 MB/小时`
  - 高峰小时段集中在 `2026-04-12 13:00 ~ 20:00`
  - 因此当前日志增长不是稳定匀速，而是“平时较低，高峰时段明显暴涨”的模式。
...
## 2026-04-02 18:20 - 系统参数保存流程改造（保存即确认重启） + 遗留风险记录
...
1. **系统参数保存交互改造（前端）**
   - 在“系统参数保存”时增加确认弹窗：
     - 确认：保存配置并立即重启系统
     - 取消：不保存配置
   - 目的：避免“保存后未重启导致运行态与配置态不一致”的误解。

2. **版本缓存控制**
   - 更新 `app.js` 版本号，确保前端获取最新交互逻辑。

### 遗留问题（已记录，暂不修复）
- **波动异常值偶发偏大（如 KOMA 104%）**：
  - 初步判断与全量流场景下个别异常 tick/低流动性数据有关。
  - 当前策略：先记录，不在本次迭代内处理。
  - 后续建议：增加异常值过滤与推送前防护（如单 tick 异常跳变阈值、最小有效价格保护、跨样本确认）。
...
## 2026-04-12 ~ 2026-04-13 - 波动侦测、通知与部署流程一轮修复整理

### 主要修复

#### 1. 波动通知内容增强
- 让波动告警带上价格区间，格式更直观
- 修复小币种价格显示成 `0 → 0` 的问题，极小价格改为更多小数位显示
- 波动通知现在能正确传递起始价、结束价和涨跌幅

#### 2. 波动监控架构拆分
- 价格监控和波动侦测继续保持独立
- 修复波动模式下的数据流、触发和展示链路
- 全局模式下的 Alpha 全量币种映射问题持续排查并修正

#### 3. 静默期与重复通知问题
- 修复波动告警只在发送成功后才进入静默期的问题
- 改为无论发送成功与否都立即进入静默期，避免重复轰炸
- 同步把静默期状态改成立即落盘，减少重启丢失风险

#### 4. 监控范围修正
- 修正波动侦测只覆盖 `enabled: true` 币种的问题
- 波动侦测现在按监控列表全量币种运行，不再受价格监控开关影响

#### 5. 通知测试与配置体验
- 保留 `POST /api/notification/test` 作为前端“测试通知”功能
- 增加 Bark 开关校验，没配好时不允许误开启
- Bark 通知文案改成更简洁的极简格式

#### 6. 调试接口清理
- 删除了遗留的 `GET /api/debug/price-buffer`
- 确认没有其他 `/api/debug/*` 调试口残留

#### 7. 部署脚本重做
- 将 `deploy.sh` 拆成安装、更新、卸载三套流程
- 更新时保留配置、历史和 PM2 运行方式，减少误覆盖

### 版本与发布
- 更新到 `v1.1.6`
- 创建了对应 release commit 和 tag

### 备注
- `GET /api/status`、`GET /api/cache/status` 仍然是有意开放的状态接口
- `POST /api/notification/test` 不是调试残留，是前端按钮在正常使用的功能接口
